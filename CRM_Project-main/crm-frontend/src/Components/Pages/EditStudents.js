...
export const EditStudents = ({ studentsId, onClose }) => {
    const [formData, setFormData] = useState({
      ...
      const fetchStudents = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/students/${studentsId}`);
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        try {
          await axios.put(`http://localhost:5000/students/${studentsId}`, formData);
